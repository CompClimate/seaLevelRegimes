...
def compute_base_label_entropy(labels, emb_params, n_clusters,
                               baselab_id, num_members, entropy_dir, method):
    """
    ...
    Args:
        ...
        baselab_id (int): Base label ID.
        num_members (int): Number of ensemble members.
        entropy_dir (str): Directory to save the entropy output files.
        method (str): Method to use for overlap calculation.
    """
    # Create output directory if it doesn't exist
    if method == 'hungarian':
        output_dir = f'{entropy_dir}/nclusters_{n_clusters}/{method}/base_labels'
    else:
    ...
    for id, nkey in enumerate(emb_params):
        filename = f"entropy_base_label_id_{baselab_id}_nn_{nkey[0]}_md_{nkey[1]}.csv"
        output_filename = f'{output_dir}/{filename}'

        if not os.path.exists(output_filename):
            df_ent = af.get_ent_for_all_params(labels=labels, param_id=id, baselab_id=baselab_id, num_members=num_members, method=method)
...
def run_base_label_entropy(n_clusters:int, resolution:str, field:str,
                           baselab_id:int, num_members:int=20, method='hungarian'):
    """
    ...
    print(f"{' '*5}• Member base label ID = {baselab_id}")
    ...
    compute_base_label_entropy(labels=labels, emb_params=emb_params, n_clusters=n_clusters,
                               baselab_id=baselab_id, num_members=num_members,
                               entropy_dir=entropy_dir, method=method)
    
    # Record the end time
    end_now = nf.time_now()
    print(f"Process finished at: {end_now}")

    print("\n-------------- FINISHED COMPUTING CLUSTER BASE LABEL ENTROPY ----------------\n\n")
...
if __name__ == "__main__":
    ...
    baselab_id = int(sys.argv[2])
    num_members = 20 # Number of ensemble members. Defaults to 20.
    resolution = 'p125' 
    field = 'statics'  # for 'statics' or 'dynamics', depending on your data
    method = 'hungarian'  # for 'hungarian' or 'jaccard', depending on your data
    
    # Call and run the entropy base label main function
    run_base_label_entropy(n_clusters=n_clusters, resolution=resolution, field=field,
                           baselab_id=baselab_id, num_members=num_members, method=method)
