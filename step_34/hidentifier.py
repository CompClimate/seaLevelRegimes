...
bvb_dir = '/group/maikesgrp/laique/NOAA/BVB'
...
def save_regimes_as_DataArray(regimes:list[np.ndarray], n_clusters:int,
    ...
    for id, clusters in enumerate(regimes):
        ...
        nn, md = af.UMAP_NNS_MDS[id]
        
        # Define the output filename
        filename = f"regimes_from_md_{md}_nn_{nn}_nc_{n_clusters}.nc"
...
def run_regimes_identifier(n_clusters:int, resolution:str, field:str, num_members:int=20, method:str='hungarian'):
    """
    ...
    Args:
        ...
        method (str, optional): Clustering method used for entropy calculation. Defaults to 'hungarian'.
    """ 
    ...
    regimes_dir = f'{base_dir}/CM4X-{resolution}/outputs/{field}/regimes'
    
    # Create output directory if it doesn't exist
    if method == 'hungarian':
        output_dir = f'{regimes_dir}/nclusters_{n_clusters}/{method}'
    else:
    ...
    # Load base label ids
    if method == 'hungarian':
        input_dir = f'{entropy_dir}/nclusters_{n_clusters}/{method}/min_entropy'
    else:
    ...
    # Get NEMI clusters
    regimes = [nf.assess_overlap_method(ens_labels=labels[id, :, :], label_id=label_ids[id], 
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python regimes_hidentifier.py <n_clusters>")
        sys.exit(1)
    ...
    method = 'hungarian'  # Clustering method used for entropy calculation. Defaults to 'hungarian'.
