...
def load_base_label_entropy(entropy_dir:str, n_clusters:int, method:str):
    """
    ...
    Args:
        ...
        method (str): Method for calculating overlap, used for naming the input files.

    Returns:
        OrderedDict: A dictionary of minimum entropy data sorted by keys.
    """
    if method == 'hungarian':
        data_dir = f'{entropy_dir}/nclusters_{n_clusters}/{method}/base_labels'
    else:
        data_dir = f'{entropy_dir}/nclusters_{n_clusters}/base_labels'
    filenames = glob.glob(f'{data_dir}/entropy_base_label_id_*.csv')
...
def concatenate_base_label_entropy(n_clusters:int, emb_params:list[tuple],
                                   entropy_dir:str, method:str):
    """
    ...
    Args:
        ...
        method (str): Method for calculating overlap, used for naming the output files.
    """
    # Load ncluster-based entropy data for all base label IDs.
    af.log_info(f"Loading entropy CSV data associated with #clusters = {n_clusters} ...")
    base_label_dict = load_base_label_entropy(entropy_dir, n_clusters, method)
    
    # Define output directory
    if method == 'hungarian':
        output_dir = f'{entropy_dir}/nclusters_{n_clusters}/{method}/mean_entropy'
    else:
    ...
    for nn, md in emb_params:
        ...
        filtered_values = [value for (a, b, c), value in base_label_dict.items() if b == nn and c == md]
...
def export_min_entropy_base_labels(n_clusters:int, entropy_dir:str, method:str):
    """
    ...
    if method == 'hungarian':
        data_dir = f'{entropy_dir}/nclusters_{n_clusters}/{method}/mean_entropy'
    else:
    ...
    # Define output directory
    if method == 'hungarian':
        output_dir = f'{entropy_dir}/nclusters_{n_clusters}/{method}/min_entropy'
    else:
...
def assemble_base_label_entropy(n_clusters:int, resolution:str, field:str,
                                num_members:int=20, method='hungarian'):
    """
    ...
    Args:
        ...
        method (str, optional): Method for calculating overlap. Defaults to 'hungarian'.
    """ 
    # List of UMAP key parameter combinations - minimum distances & nearest neighhbors.
    emb_params = af.UMAP_NNS_MDS
    
    print("\n-------------- STARTING PROCESSING CLUSTER BASE LABEL ENTROPY ----------------\n")
    ...
    clust_dir = f'{base_dir}/CM4X-{resolution}/outputs/{field}/clusterings'
    entropy_dir = f'{base_dir}/CM4X-{resolution}/outputs/{field}/entropy'

    # Load and concatenate ncluster-based entropy data for all base label IDs.
    concatenate_base_label_entropy(n_clusters=n_clusters, emb_params=emb_params, entropy_dir=entropy_dir, method=method)
    
    # Export base labels with minimum entropy to a text file
    export_min_entropy_base_labels(n_clusters=n_clusters, entropy_dir=entropy_dir, method=method)
    ...
    print("\n-------------- FINISHED PROCESSING CLUSTER BASE LABEL ENTROPY ----------------\n")



def process_clusters_entropy(clust_dir:str, n_clusters:int, lst:list, emb_params:list[tuple], 
                             entropy_dir:str, num_members:int=20, method:str='hungarian'):
    """
    ...
    # Calculate entropy
    if method == 'hungarian':
        output_dir = f'{entropy_dir}/nclusters_{n_clusters}/{method}'
    else:
    ...
    for id in range(n_params):
        ...
        if not os.path.exists(output_filename):
            ...
            sorted_overlap = af.get_overlap_method(ens_labels=labels, id=lst[id], num_members=num_members, method=method)
...
def run_clusters_entropy(n_clusters:int, resolution:str, field:str, num_members:int=20, method='hungarian'):
    """
    Main function to execute the entropy calculation for a given cluster.
    ...
    if method == 'hungarian':
        input_filename = f"{entropy_dir}/nclusters_{n_clusters}/{method}/min_entropy/{filename}"
    else:
    ...
    process_clusters_entropy(clust_dir=clust_dir, n_clusters=n_clusters, lst=lst, emb_params=emb_params, 
                             entropy_dir=entropy_dir, num_members=num_members, method=method)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python hungarian_entropy.py <n_clusters>")
        sys.exit(1)
    ...
    resolution = 'p125' 
    field = 'statics'  # Or 'dynamics', depending on your data
    method = 'hungarian'
    
    
    # Call and run the assembler of base label entropy
    assemble_base_label_entropy(n_clusters=n_clusters, resolution=resolution, field=field, 
                                num_members=num_members, method=method)
    
    # Call and run the clusters entropy processor
    run_clusters_entropy(n_clusters=n_clusters, resolution=resolution,
                         field=field, num_members=num_members, method=method)
