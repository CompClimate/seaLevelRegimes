...
for MEMBER in "${ENSEMBLES[@]}"; do
    echo "==============================================================================================================="
    echo "Submitting a DBSCAN Job of ${MEMBER}-Member Ensemble to SLURM with the Following Configuration ..."
    MEMBER_SET=("${MEMBER}")
    sbatch --job-name=DB:${MEMBER}:${DATA_RES}:MD${MD}:NN${NN} cpu_dbscan.sh \
        "${MEMBER_SET[*]}" "${MD}" "${NN}" \
        "${EPSILON}" "${MIN_SAMPLES}" \
